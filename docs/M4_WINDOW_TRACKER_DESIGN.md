...
| `EVENT_SYSTEM_FOREGROUND` | **да** (пересмотрено) | полное `enum_windows()`. Изначально было «нет» — z-order маске не был нужен, пока она строилась как чистое объединение rect'ов (ADR-004 §4.7). После живого репорта пользователя (переключение окон не возвращало стикер) окклюдер стал вычитать окна выше по z-order (`occluder_rects_for`/`occluders::subtract_rects`) — смена переднего плана меняет видимую площадь окклюдера и обязана будить пересчёт |
