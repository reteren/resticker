...
- ~~Потоковый режим для анимаций >300 кадров/>256MB — decode-слой честно
  отказывает (`TooManyFrames`/`TooLargeForAtlas`)~~ — сделано позже,
  см. ROADMAP.md M5a: `rst_media::animation::StreamingAnimation` декодирует
  такие файлы по кадру вместо отказа, координатор переключается на неё
  автоматически. Отдельного UI-сообщения об ошибке в итоге не понадобилось
  (файл просто играет в потоковом режиме, не показывая пользователю разницы).
