...
const GLYPHS: &[(char, Glyph)] = &[
    ...
    (
        'Ё',
        Glyph {
            // Найдено независимым ревью (2026-08-04): тело буквы сидело на
            // 1px ниже базовой линии остальных букв (кончалось row7 вместо
            // row6). Точки сжаты в одну строку (как бревис у 'Й'), тело —
            // тот же контур 'Е', сдвинутый на строку вверх, база — row6.
            rows: [
                0b0101_0000,
                0b1111_1000,
                0b1000_0000,
    ...
    (
        'ё',
        Glyph {
            // См. 'Ё' выше — та же правка: точки сжаты в одну строку, тело
            // 'е' (row0 у него пустой — x-height буква ниже прописных)
            // занимает те же row1-6, что и у 'е', база — row6.
            rows: [
                0b0101_0000,
                0b0111_0000,
                0b1000_1000,
    ...
    // Двоеточие — подписи времени видео-перемотки (`m:ss` / `h:mm:ss`,
    // video_timeline.rs): без собственного глифа `glyph_for(':')` вернул бы
    // FALLBACK-квадрат, и «1:23» читалось бы как «1□23». Две точки в тех же
    // рядах, что тело цифр (1-2 и 5-6), шаг 4 — вдвое уже цифры: в подписи
    // «12:34» на пару двоеточий не тратится лишняя ширина.
    (
        ':',
        Glyph {
            rows: [
...
mod tests {
    ...
    #[test]
    fn colon_is_two_dots_within_digit_rows() {
        // Подпись времени: двоеточие обязано быть двумя раздельными точками
        // в рядах тела цифр (не сливаться в палку) — иначе «1:23» нечитаемо.
        let g = glyph_for(':');
        assert_ne!(g, FALLBACK);
        assert_eq!(g.advance, 4);
        let lit: Vec<u8> = [0b0011_0000, 0b0011_0000, 0, 0, 0b0011_0000, 0b0011_0000]
            .into_iter()
            .collect();
        assert_eq!(g.rows[1..7], lit[..]);
        assert_eq!(g.rows[0], 0);
        assert_eq!(g.rows[7], 0);
    }

    #[test]
    fn time_caption_strings_rasterize_with_colon() {
        // m:ss / h:mm:ss-строки с новым глифом: каждая даёт закрашенные
        // пиксели (FALLBACK-квадрат давал бы пустую серединку, но тут —
        // просто проверка, что растеризация не падает и не пуста).
        for s in ["0:00", "1:05", "59:59", "1:00:00"] {
            let (rgba, w, h) = rasterize(s, [255, 255, 255], 1);
            assert_eq!((w, h), (text_size(s).0 as u32, 8));
            assert!(rgba.iter().any(|&b| b != 0), "строка {s:?} должна быть закрашена");
        }
    }
