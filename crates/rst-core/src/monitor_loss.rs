...
impl MonitorLossTracker {
    ...
    /// Есть ли что-то, ради чего автомат надо будить по таймеру.
    ///
    /// Пусто — значит ни один монитор не потерян и ни один таймер не тикает:
    /// вызывающему НЕ нужен периодический тик, и в покое программа обязана
    /// спать (SPEC.md §13: «в полном покое программа не должна просыпаться
    /// вообще»). До 2026-09-07 координатор будил себя раз в секунду
    /// безусловно — замер живого процесса это и показал.
    ///
    /// Стикеры, помеченные `on_user_edit`, сюда не считаются: это память об
    /// уже случившихся правках, никакого таймера за ней нет.
    pub fn has_pending_losses(&self) -> bool {
