...
/// таблица не меняется.
static TABLE: OnceLock<[Vec<Preset>; 7]> = OnceLock::new();
...
fn build_table() -> [Vec<Preset>; 7] {
    [
        vec![
            // 2 окна: половины и «главное + сосед» в обе стороны по обеим
            // осям. Седьмой была «главное по центру с полосой снизу»
            // (0.25..0.75 сверху, полная ширина снизу) — снята по запросу
            // пользователя 2026-09-06: она единственная оставляла экран
            // пустым по бокам, то есть предлагала выбросить место, ради
            // которого тайлинг и нужен.
            Preset {
...
mod tests {
    ...
    /// На каждое количество окон — несколько РАЗНЫХ раскладок (одинаковые
    /// сетки не годятся: лента должна предлагать выбор), каждая — ровно с
    /// `count` слотами. Точное число не проверяется: у двух окон их шесть
    /// (седьмая снята, см. `build_table`), у остальных семь, и прибивать
    /// это число гвоздём значило бы ломать тест на каждой правке состава.
    #[test]
    fn each_count_offers_distinct_layouts_of_the_right_size() {
        for count in 2..=8 {
            let presets = presets_for(count);
            assert!(presets.len() >= 6, "count={count}: {}", presets.len());
    ...
    fn slots_never_overlap_for_any_work_area_and_realistic_gap() {
        ...
        // Ожидаемые числа считаются по самой таблице, а не вписаны
        // константой: состав раскладок правится руками (у двух окон их
        // шесть, у остальных семь), и магическое число ломалось бы на
        // каждой такой правке, ничего не проверяя по сути. Смысл проверки
        // в другом — что перебор действительно дошёл до каждой раскладки.
        let expected_calls: usize = 5 * 5 * (2..=8).map(|c| presets_for(c).len()).sum::<usize>();
        let expected_pairs: usize = 5
            * 5
            * (2..=8)
                .map(|c| presets_for(c).len() * c * (c - 1) / 2)
                .sum::<usize>();
        assert_eq!(apply_calls, expected_calls, "полнота перебора");
        assert_eq!(pair_checks, expected_pairs, "полнота пар");
    }
