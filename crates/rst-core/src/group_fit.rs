...
mod tests {
    ...
    fn every_layout_fits_minimums_without_overlap() {
        ...
        // Ожидаемое число считается по самой таблице раскладок, а не
        // константой: состав правится руками (у двух окон их шесть, у
        // остальных семь), и вписанное число ломалось бы на каждой правке,
        // ничего не проверяя. Смысл — что перебор дошёл до каждой пары.
        let expected: usize = 2
            * 3
            * (2..=8)
                .map(|c| presets_for(c).len() * c * (c - 1) / 2)
                .sum::<usize>();
        assert_eq!(pair_checks, expected, "полнота перебора");
    }
