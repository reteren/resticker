...
//
// Фаза ПЕРЕХВАТА (третий аргумент `true`), а не всплытия, и это не вкусовщина.
// Клик по самой надписи внутри поля («Not set», чипы клавиш) сначала попадал в
// обработчик поля, тот включал запись и заменял содержимое — надпись
// отсоединялась от документа. К этому обработчику событие приходило уже с
// ОТСОЕДИНЁННЫМ `e.target`, у которого нет предков: `closest('.hotkeyField')`
// возвращал `null`, и запись отменялась в тот же миг. Снаружи это выглядело
// как мёртвая зона ровно по надписи — клик мимо неё, по пустому месту поля,
// работал (репорт пользователя со скриншотом, 2026-09-17; замер: клики на
// x=580..660 включали запись, на 680..700 — нет).
//
// В перехвате обработчик отрабатывает ДО обработчика поля, пока узел ещё в
// документе, и `closest` отвечает правду.
