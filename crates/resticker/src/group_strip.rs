...
pub struct StripPanel {
    ...
    ///
    /// Читают только тесты: координатору хватает самой панели, а раскладку
    /// проверять больше нечем — счётчики и есть то, что утверждают тесты
    /// «все карточки построены» и «на вырожденном экране не построено ни
    /// одной». Глушилка узкая, на два поля: модуль целиком под
    /// `allow(dead_code)` прятал бы и настоящие находки.
    ...
    /// Читают только тесты — см. `total_cards`.
    #[cfg_attr(not(test), allow(dead_code))]
