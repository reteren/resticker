...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    GetCursorPos, GetMessageW, HICON, IDI_APPLICATION, LoadIconW, MSG, PostMessageW,
    PostQuitMessage, RegisterClassExW, SetWindowLongPtrW, TranslateMessage, WM_APP, WM_CLOSE,
    WM_CONTEXTMENU, WM_DESTROY, WM_LBUTTONUP, WM_RBUTTONUP, WNDCLASSEXW, WS_EX_NOACTIVATE,
    WS_OVERLAPPED,
};
...
pub enum TrayEvent {
    /// Клик левой кнопкой прямо по иконке.
    Activate,
    /// Правая кнопка (или клавиша контекстного меню) по иконке: пора
    /// показать меню в точке `x`,`y` — экранные физические пиксели.
    ///
    /// Сам модуль меню больше не рисует. Всплывающее меню Win32 красится
    /// системой, и owner-draw дотягивался лишь до плашек пунктов: ни
    /// прозрачности, ни размытия подложки, ни настоящих скруглений у него
    /// нет, а градиент GDI кладёт полосами. Пятьсот строк такой отрисовки
    /// давали «почти похоже» и всё равно выбивались из программы (репорт
    /// пользователя 2026-09-17). Меню рисует webview-окно самой программы —
    /// тем же CSS, что и окно настроек.
...
impl TrayIcon {
    ...
    pub fn new(tooltip: &str) -> Result<(Self, Receiver<TrayEvent>), Win32Error> {
        let (tx, rx) = mpsc::channel::<TrayEvent>();
        let (ready_tx, ready_rx) = mpsc::channel::<Result<SendHwnd, Win32Error>>();
        let tooltip = tooltip.to_string();

        let thread = thread::spawn(move || run_message_loop(tooltip, tx, ready_tx));
    ...
    match msg {
        WM_TRAYICON => {
            ...
            match event {
                ...
                // Координаты берём у КУРСОРА, а не из `lparam`: для
                // сообщений трея там лежит позиция иконки в системе
                // уведомлений, а не точка клика.
                WM_RBUTTONUP | WM_CONTEXTMENU => {
                    let mut pt = POINT::default();
                    // SAFETY: `pt` — валидный out-параметр на весь вызов.
                    let ok = unsafe { GetCursorPos(&mut pt) }.is_ok();
                    if ok {
                        with_state(hwnd, |s| {
                            let _ = s.tx.send(TrayEvent::ContextMenu { x: pt.x, y: pt.y });
                        });
                    }
                }
                _ => {}
...
mod tests {
    ...
    fn create_then_drop_destroys_window() {
        let (tray, _rx) = TrayIcon::new("test").expect("создание трея");
        let hwnd = tray.hwnd;
